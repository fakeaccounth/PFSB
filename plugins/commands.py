...
async def start(client, message):
    ...
    if len(message.command) != 2:
        buttons = [[
            InlineKeyboardButton('🔍 sᴜᴘᴘᴏʀᴛ ɢʀᴏᴜᴘ', url='https://t.me/Noob_Studio_Chat'),
            InlineKeyboardButton('🤖 Owner', url='https://t.me/NC_Owner_bot')
            ]]
        if CLONE_MODE == True:
    ...
    if not files_:
        ...
        try:
            ...
            title = '[@NooB_Corporations]  ' + ' '.join(filter(lambda x: not x.startswith('[') and not x.startswith('@'), file.file_name.split()))
